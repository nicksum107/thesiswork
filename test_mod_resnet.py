...
def load_model(model_type, model_dir, clipping, device, aggr, dohisto, collapsefunc):
    if clipping > 0:
    ...
    if 'bagnet17' in model_type:
        ...
        model = nets.resnet.resnet50(clip_range=clip_range, aggregation=aggr, dohisto=dohisto, collapsefunc=collapsefunc)
...
model = load_model(args.model, args.model_dir, args.clip, device, args.aggr, dohisto=False, collapsefunc=None)
attacker = PatchAttacker(model, [0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010],patch_size=args.patch_size,step_size=0.05,steps=500)#750

model2 = load_model(args.model, args.model_dir, args.clip, device, args.aggr, dohisto=False, collapsefunc='max')


# here
# test_batch = torch.zeros((1, 3, img_size, img_size))
# out = model2(test_batch)
# # normalizer = transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
# # norm = normalizer.forward(test_batch)
# # print(norm)
# # out = model2(norm)
# print(out)
# test_batch2 = torch.ones((1,3,img_size, img_size)) 
# test_batch2[0,0,:,:] = 0.4914
# test_batch2[0,1,:,:] = 0.4822
# test_batch2[0,2,:,:] = 0.4465
# # print(test_batch2)
# out2 = model2(test_batch2)
# print(out2)
# nums = torch.linspace(0, 1, 16)

# print(test_batch.shape)
# test_batch = test_batch.to(device)
# for i in range(len(nums)):
#     test_batch[0,0,int(img_size/2), int(img_size/2)] = nums[i]
#     for j in range(len(nums)):
#         test_batch[0,1,int(img_size/2), int(img_size/2)] = nums[j]
#         for k in range(len(nums)):
#             test_batch[0,2,int(img_size/2), int(img_size/2)] = nums[k]
#             out = model2(test_batch)
#             print(out)
...
for data, labels in tqdm(val_loader):
    ...
    if int(im_number/16) % 16 == 0:
        ...
        print("Attacked accuracy:", np.mean(error_list))
