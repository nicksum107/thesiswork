...
class DualPatchAttacker:
    def __init__(self, model1, model2, mean, std, image_size=244,epsilon=1,steps=500,step_size=0.05,patch_size=31,random_start=True):
    ...
    def perturb(self, inputs, labels, loc=None,random_count=1):
        ...
        for _ in range(random_count):
            ...
            # 
            # print('train single')
            for step in range(self.steps//3*2+1):
                #attacking two netwroks
                x.requires_grad_()

                # if step / 100 == 6:
                #     print('stepdown') 
                #     self.step_size = (size/2) / self.std
                #     self.step_size=self.step_size[None, :, None, None].cuda()

                # output = self.model1(torch.where(mask, x, x_init))
                output = self.model2(torch.where(mask, x, x_init))
                # output = output/3
                ...
                x = torch.max(torch.min(x, x_init + self.epsilon_cuda), x_init - self.epsilon_cuda)#.detach()
                x = torch.min(torch.max(x, self.lb), self.ub).detach().clone()
            # print('fine tune with both')
            for step in range(self.steps//3+1):
                #attacking two netwroks
                x.requires_grad_()
...
class AdaptivePatchAttacker:
    def __init__(self, model, mean, std, image_size=244,epsilon=1,steps=500,step_size=0.05,patch_size=31,random_start=True, ratio=0): # 0 = nonadaptive
    ...
    def perturb(self, inputs, labels, loc=None,random_count=1):
        ...
        for _ in range(random_count):
            ...
            prev_act_grad = []
            memory = 1 # iterations to remember the act grads
            for step in range(self.steps+1):
                x.requires_grad_()
                output, masked_activation_list = self.model(torch.where(mask, x, x_init))
                loss_ind = torch.nn.functional.cross_entropy(input=output, target=labels,reduction='none')
                loss = loss_ind.sum()
                # print(loss)

    
                activation_loss = torch.tensor(0.).to(loss.device)
                mseloss = torch.nn.MSELoss()#reduction='sum'
                for a in masked_activation_list:
                    z = torch.zeros(a.shape).to(a.device)
                    activation_loss += mseloss(z, a)
                print(activation_loss)
                act_loss_grads = torch.autograd.grad(activation_loss, x, retain_graph=True)[0] # keep the graph for the loss calculation

                # if prev_act_grad == None:
                #     prev_act_grad = act_loss_grads
                # else: 
                #     prev_act_grad = prev_act_grad + act_loss_grads
                
                # prev_act_grad.append(act_loss_grads)
                # if len(prev_act_grad) > memory: 
                #     prev_act_grad = prev_act_grad[1:]
                # avg_grad = act_loss_grads # do the current loss grad double 
                # for g in prev_act_grad: 
                #     avg_grad += g 
                # avg_grad /= len(prev_act_grad) + 1

                grads = torch.autograd.grad(loss, x,retain_graph=False)[0]
                # if step > 50:
                grads -= self.ratio * act_loss_grads
